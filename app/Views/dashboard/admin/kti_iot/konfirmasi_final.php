    ...
    <tbody>
      ...
      <?php foreach ($list_tim_full_paper as $tim) : ?>
        <tr>
          ...
          <td>
            <a href="" class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#modalTerimaData" onclick="document.getElementById('buttTerimaData').href = '<?= base_url() ?>/dashboard/Admin_kti_iot/verify_konfirmasi_final/<?= $tim['iot_id'] ?>/1';">Terima</a>
            <a href="" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#modalTolakData" onclick="document.getElementById('buttTolakData').href = '<?= base_url() ?>/dashboard/Admin_kti_iot/verify_konfirmasi_final/<?= $tim['iot_id'] ?>/0';">Tolak</a>
          </td>
        </tr>
        <?php $i++; ?>
      <?php endforeach; ?>
